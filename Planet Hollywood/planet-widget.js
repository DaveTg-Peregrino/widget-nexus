...
:root {
  ...
  --nx-user-bubble-bg: #C3DDE5;
  --nx-user-bubble-border-color: #9EC0CB;
  --nx-send-button-bg: #8E7143;
  --nx-send-button-border-color: transparent;
  --nx-send-button-icon-color: #FFFFFF;

  /* Gradient */
  --nx-grad-primary: linear-gradient(90deg,#004A63 0%,#8E7143 100%);
...
.nx-msg--user {
  ...
  color: var(--nx-user-bubble-text-color);
  align-self: flex-end;
}
.nx-msg--system {
  align-self: center;
...
@keyframes nexus-glow-intense {
    ...
    25% { 
        box-shadow: 0 2px 25px rgba(0, 74, 99, 0.6), 0 0 35px rgba(0, 74, 99, 0.4);
        transform: scale(1.02);
    }
    50% { 
        box-shadow: 0 4px 30px rgba(0, 74, 99, 0.8), 0 0 40px rgba(0, 74, 99, 0.6), inset 0 0 20px rgba(0, 74, 99, 0.2);
...
@keyframes nexus-attention-ring {
    0% { 
        box-shadow: 0 0 0 0 rgba(0, 74, 99, 0.7);
        transform: scale(1);
    }
    25% {
        box-shadow: 0 0 0 8px rgba(0, 74, 99, 0.5);
        transform: scale(1.02);
    }
    50% {
        box-shadow: 0 0 0 16px rgba(0, 74, 99, 0.3);
        transform: scale(1.05);
    }
    75% {
        box-shadow: 0 0 0 12px rgba(0, 74, 99, 0.2);
        transform: scale(1.02);
    }
    100% {
        box-shadow: 0 0 0 0 rgba(0, 74, 99, 0);
        transform: scale(1);
    }
}
...
.nexus-notification-dot {
    ...
    box-shadow: 0 2px 8px rgba(243, 111, 33, 0.4);
}
...
.nx-chat-button:hover {
    ...
    box-shadow: 0 4px 20px rgba(0, 74, 99, 0.3);
}
`;
    ...
    const config = {
        ...
        theme: {
            // Chat Button (Icono)
            '--nx-button-bg': '#DDCBDE',
            '--nx-button-icon-color': '#4B1D50',

            // Notification Dot (Notificacion)
            '--nx-notification-dot-bg': '#FFA409',
            ...
            '--nx-agent-bubble-bg': '#FFFFFF',
            '--nx-agent-bubble-border-color': '#E0E0E0',
            '--nx-user-bubble-bg': '#DDCBDE',
            '--nx-user-bubble-border-color': '#C4B2C6',
            '--nx-user-bubble-text-color': '#4B1D50',

            // Send Button (Boton ok)
            '--nx-send-button-bg': '#BDBDBD',
            '--nx-send-button-icon-color': '#FFFFFF'
        }
    };
    ...
    let deviceIdKey = 'nexus_device_id'; // To be made unique in initWidget
    let threadIdKey = 'nexus_thread_id'; // To be made unique in initWidget
    let initializationPromise = null; // To handle concurrent initializations
    
    // Batching state for user messages
    let messageQueue = '';
    let tempMessageIds = [];
    let sendTimer = null;
    const BATCH_SEND_DELAY = 5000; // 5 seconds

    // A flag to prevent concurrent message sending
    let isSending = false;
        ...
        widget.innerHTML = `
            ...
            <div id="nexus-widget-chat" class="nx-chat nx-hidden" style="${config.position === 'bottom-right' ? 'right: var(--nx-space-4);' : 'left: var(--nx-space-4);'}">
                ...
                <div id="nexus-widget-end-chat-container" style="padding: 8px; text-align: center; background: var(--nx-gray-100);">
                    <button id="nexus-widget-end-chat" style="background: none; border: none; color: #004A63; cursor: pointer; font-size: 0.8rem; font-weight: bold;">End Chat</button>
                </div>
    ...
    function initWidget(customerConfig) {
        ...
        // Make localStorage keys unique per channel to avoid conflicts
        deviceIdKey = `nexus_device_id_${config.channel}`;
        threadIdKey = `nexus_thread_id_${config.channel}`;

        // Generate or retrieve device ID
        deviceId = localStorage.getItem(deviceIdKey) || generateDeviceId();
    ...
    function toggleWidget() {
        ...
        if (isOpen) {
            ...
            if (localStorage.getItem(threadIdKey) && !threadId) {
    ...
    async function initializeSessionAndThread(createIfNotFound = true) {
        ...
        try {
            const storedThreadId = localStorage.getItem(threadIdKey);
            if (storedThreadId) {
    ...
    function addMessageSafely(role, content, timestamp, messageId) {
        ...
        // Skip concatenated messages that come from WebSocket ONLY during active batching
        if (isBatchingActive && role === 'customer' && content.includes('. ') && (!idStr || !idStr.startsWith('temp_'))) {
            console.log('Skipping concatenated WebSocket message during batching:', content);
            return false;
        }
    ...
    async function sendBatchedMessage() {
        console.log('🚀 sendBatchedMessage called');
        console.log('🚀 isSending:', isSending);
        console.log('🚀 messageQueue:', messageQueue);
        console.log('🚀 tempMessageIds:', tempMessageIds);
        
        if (isSending || !messageQueue) {
            console.log('🚀 Exiting early - isSending:', isSending, 'messageQueue empty:', !messageQueue);
            return;
        }

        isSending = true;

        const batchedContent = messageQueue;
        const batchedTempIds = [...tempMessageIds];

        console.log('🚀 About to send batched content:', batchedContent);
        console.log('🚀 About to send temp IDs:', batchedTempIds);

        // Clear the timer but DON'T reset the queue yet
        sendTimer = null;

        try {
            if (!threadId) {
                console.log('✉️ No thread found, creating a new one for batched message...');
                await initializeSessionAndThread(true);

                if (!threadId) {
                    throw new Error("Failed to create a new session. Please refresh.");
                }
                console.log('✅ New thread created, proceeding to send batched message.');
            }
            ...
            console.log('Batched message sent successfully to thread:', threadId, responseData);
            
            // Only reset the queue after successful send
            messageQueue = '';
            tempMessageIds = [];
            
            // Release the lock immediately after successful send
            isSending = false;
            ...
            // Re-queue the message if send fails to avoid losing user input
            messageQueue = batchedContent;
            tempMessageIds = batchedTempIds;
            
            // Remove all temporary messages from the failed batch
            batchedTempIds.forEach(tempId => {
            ...
            // Deactivate batching mode even on error
            isBatchingActive = false;
        }
    }
    ...
    function sendMessage() {
        ...
        console.log('📝 Current messageQueue before update:', messageQueue);
        ...
        // Always create a temporary message for UI display
        const tempMessageId = 'temp_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
        const timestamp = new Date().toISOString();
        addMessageSafely('customer', content, timestamp, tempMessageId);
        
        // Add message to the queue for batching
        if (messageQueue) {
            messageQueue += '. ' + content;
        } else {
            messageQueue = content;
            // Activate batching mode when first message is queued
            isBatchingActive = true;
        }
        tempMessageIds.push(tempMessageId);
        ...
        // Set a new timer to send the batched message
        sendTimer = setTimeout(() => {
            console.log('⏰ Timer fired, calling sendBatchedMessage with queue:', messageQueue);
            sendBatchedMessage();
        }, BATCH_SEND_DELAY);
        
        console.log('📝 Set new timer for', BATCH_SEND_DELAY, 'ms');
    }
